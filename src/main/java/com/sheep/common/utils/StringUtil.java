...
public class StringUtil {
    ...
    public static void checkIllegalAngleBracketChars(String... values) {
        ...
        for (String s : values) {
            ...
            for (char c : charArray) {
                if (illegalChars[0] == c || illegalChars[1] == c) {
                    throw new RuntimeException("参数中含有非法字符，请检查: "+StringEscapeUtils.escapeHtml(s));
                }
            }
        }
    }
    
    public static String removeAddressQuotes(String areaName){
        if(areaName != null){
            areaName = areaName.trim();
            if(areaName.length()>0&&((int)areaName.charAt(0)==65279)){ //京东地址，第一个字符是65279
                areaName = areaName.substring(1);
            }
            if(areaName.startsWith("\"")){
                areaName = areaName.substring(1);
            }
            if(areaName.endsWith("\"")&&areaName.length()>1){
